  ...
  try {
    ...
    if (req.user.role === "student") {
      ...
      query = `
        ...
        JOIN users u ON u.id = a.student_id 
        LEFT JOIN classes c ON c.id = u.class_id
        WHERE u.class_id = $1 AND a.school_id = $2
        ORDER BY a.date DESC
      `;
      params = [teacherClassId, req.user.school_id];
    } else {
      query = `
        SELECT 
          a.id, a.student_id, a.date, a.status, a.remarks, a.created_by, a.school_id,
    ...
    if (search) {
      query = query.replace(
        /\s+ORDER BY/i,
        ` AND (u.id::text ILIKE $${params.length + 1} OR u.name ILIKE $${params.length + 1} OR u.email ILIKE $${params.length + 1}) ORDER BY`
      );
      params.push(`%${search}%`);
    }
...
const editAttendance = async (req, res) => {
  ...
  try {
    const scope = req.user.role === 'teacher' ? "AND class_id = (SELECT class_id FROM users WHERE id = $5 AND role = 'teacher' AND school_id = $4)" : '';
    const { rows } = await pool.query(
      `UPDATE attendance 
       SET status=$1, remarks=$2 
       WHERE id=$3 AND school_id=$4 ${scope}
       RETURNING *`,
      [status, remarks, id, req.user.school_id, req.user.id]
    );
...
const deleteAttendance = async (req, res) => {
  const { id } = req.params;
  try {
    const scope = req.user.role === 'teacher' ? "AND class_id = (SELECT class_id FROM users WHERE id = $3 AND role = 'teacher' AND school_id = $2)" : '';
    const deleted = await pool.query(
      `DELETE FROM attendance WHERE id = $1 AND school_id = $2 ${scope} RETURNING student_id, class_id, id`,
...
const exportAttendance = async (req, res) => {
  try {
    ...
    if (req.user.role === 'teacher') {
      ...
      result = await pool.query(
        `
          ...
          ORDER BY a.date DESC
        `,
        [teacherClassId, req.user.school_id]
      );
    } else {
      result = await pool.query(`
        SELECT 
    ...
    // Stream the workbook instead of writing a permanent export into uploads.
    // This avoids disk growth and prevents one user from discovering another
    // user's temporary export by guessing a filename.
