...
const getDashboardAnalytics = async (req, res) => {
  ...
  try {
    const cacheData = await withCache(
      getAdminDashboardCacheKey({ schoolId, classId, page, limit, search, month, year }),
      async () => {
        ...
        const attValues = [schoolId];
        let attParamIdx = 2;

        if (month && month !== 'all') {
          attFilters.push(`TO_CHAR(a.date, 'FMMonth') = $${attParamIdx}`);
          attValues.push(month);
          attParamIdx++;
        }
        if (year) {
          attFilters.push(`EXTRACT(YEAR FROM a.date) = $${attParamIdx}`);
          attValues.push(year);
          attParamIdx++;
        }
        const attendanceFilter = `WHERE ${attFilters.join(' AND ')}`;

        // Fee Analytics filtering logic
        const feeFilters = ['f.school_id = $1'];
        const feeValues = [schoolId];
        let feeParamIdx = 2;

        if (month && month !== 'all') {
          feeFilters.push(`f.month = $${feeParamIdx}`);
          feeValues.push(month);
          feeParamIdx++;
        }
        if (year) {
          feeFilters.push(`f.year = $${feeParamIdx}`);
          feeValues.push(year);
          feeParamIdx++;
        }
        const feeFilter = `WHERE ${feeFilters.join(' AND ')}`;
...
// Create a professional teacher salary record with breakdown calculation
const addTeacherSalary = async (req, res) => {
  ...
  try {
    ...
    // Check if duplicate record exists for this month and year
    const existing = await pool.query(
      'SELECT id FROM teacher_salaries WHERE teacher_id = $1 AND month = $2 AND year = $3 AND school_id = $4',
      [parseInt(teacherId, 10), month, parseInt(year, 10), schoolId]
    );
    if (existing.rows.length > 0) {
      return res.status(400).json({
        success: false,
        message: `Salary record for ${month} ${year} already exists for this teacher.`
      });
    }
    ...
    const { rows } = await pool.query(
      `INSERT INTO teacher_salaries (
        ...
        deductions, advance, fine, status, payment_screenshot,
        remarks, approved_by, approved_at
    ...
    res.status(500).json({ success: false, message: error.message || "Failed to create salary record" });
  }
};

// Update salary status (Workflow: draft -> pending -> approved -> paid)
const updateSalaryStatus = async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, message: `Salary marked as ${status}`, salary });
  } catch (error) {
...
// Get School Monthly Payroll / Salary Dashboard (Admin View)
const getSalaryDashboard = async (req, res) => {
  const schoolId = req.user.school_id;
  ...
  try {
    const [statsRes, teachersRes] = await Promise.all([
      ...
      `, [schoolId, month, year]),
      pool.query(`
        SELECT COUNT(*)::int AS total_teachers FROM users WHERE school_id = $1 AND role = 'teacher'
      `, [schoolId])
    ]);

    const stats = statsRes.rows[0];
    const totalTeachers = teachersRes.rows[0].total_teachers;
    ...
    res.json({
      ...
      dashboard: {
        ...
        totalPayroll: stats.total_payroll,
        paidPayroll: stats.paid_payroll,
        pendingPayroll: stats.pending_payroll,
        paidCount: stats.paid_count,
        pendingCount: stats.pending_count,
        unbilledTeachers: Math.max(0, totalTeachers - stats.total_records)
      }
    });
  } catch (error) {
    console.error("Salary Dashboard Error:", error);
    res.status(500).json({ success: false, message: "Failed to load salary dashboard" });
  }
};
