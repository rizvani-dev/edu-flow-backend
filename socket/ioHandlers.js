...
function registerSocketHandlers(io, pool) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join', async (payload = {}, ack) => {
      ...
      try {
        const userData = socket.data.authUser;
    ...
    socket.on('markSeen', async ({ senderId, messageIds }, ack) => {
      const receiverId = Number(socket.userId);
    ...
    socket.on('reactAnnouncement', async ({ announcementId, emoji }) => {
      const userId = Number(socket.userId);
