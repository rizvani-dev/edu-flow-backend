...
// Socket events are write-capable. Never trust a user id supplied by the browser.
...
const reminderInterval = setInterval(async () => {
  try {
    ...
    const pendingFeeRows = await pool.query(`
      ...
      FROM fees f
      JOIN users u ON u.id = f.student_id AND u.school_id = f.school_id
      WHERE LOWER(COALESCE(f.status, 'unpaid')) IN ('unpaid', 'overdue', 'rejected')
        AND (
...
server.listen(PORT, async () => {
  ...
  try {
    ...
    await pool.query(`
      ...
      ON attendance (school_id, class_id, date DESC)
    `);
    ...
    await pool.query(`
      CREATE INDEX IF NOT EXISTS idx_results_school_class_subject
      ON results (school_id, class_id, subject)
    `);
    ...
    // --- EduFlow Advanced Schema Enhancements ---
    // 1. Fee status constraint update (allow unpaid, pending, paid, partial, overdue, rejected)
    await pool.query(`
      DO $$
      BEGIN
        ALTER TABLE fees DROP CONSTRAINT IF EXISTS fees_status_check;
        ALTER TABLE fees ADD CONSTRAINT fees_status_check 
          CHECK (status IN ('unpaid', 'pending', 'paid', 'partial', 'overdue', 'rejected'));
      EXCEPTION WHEN OTHERS THEN
        NULL;
      END $$;
    `);

    // 2. School Fee Structures table
    await pool.query(`
    ...
    await pool.query(`
      ...
      ADD COLUMN IF NOT EXISTS overtime NUMERIC(12,2) DEFAULT 0,
    ...
    // Update teacher_salaries status constraint if needed
    await pool.query(`
      DO $$
      BEGIN
        ALTER TABLE teacher_salaries DROP CONSTRAINT IF EXISTS teacher_salaries_status_check;
      EXCEPTION WHEN OTHERS THEN NULL;
      END $$;
    `);

    // Safe unique constraint for teacher salary (one record per teacher per month and year)
    await pool.query(`
      DO $$
      BEGIN
        IF NOT EXISTS (
          SELECT 1 FROM pg_constraint WHERE conname = 'uq_salary_teacher_month_year'
        ) THEN
          ALTER TABLE teacher_salaries ADD CONSTRAINT uq_salary_teacher_month_year UNIQUE (teacher_id, month, year);
        END IF;
      EXCEPTION WHEN OTHERS THEN NULL;
      END $$;
    `);

    // 4. Fee Payment Requests month/year/timestamps & duplicate prevention
    await pool.query(`
    ...
    `);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_audit_logs_school ON audit_logs (school_id, created_at DESC)`);
